...
const SCREENSHOT_MODE = ['all', 'failures', 'none'].includes(cfg('screenshot_mode', 'failures')) ? cfg('screenshot_mode', 'failures') : 'failures';
const TIMEOUT_MINUTES = Math.max(1, Number(cfg('timeout_minutes', 30)) || 30);

function log(message) { console.log(`[website-tester] ${message}`); }
...
async function runTest() {
  ...
  const result = { started_at: startedAt, ended_at: null, status: 'FAIL', website_url: WEBSITE_URL, timezone: TIMEZONE, http_status: null, final_url: null, page_title: null, h1_count: 0, visible_links: 0, visible_images: 0, broken_images: 0, console_errors: 0, failed_requests: 0, load_time_ms: null, screenshot: null, failure_reasons: [] };
  const consoleErrors = [];
  const failedRequests = [];
  const browser = await chromium.launch({ headless: true, executablePath: process.env.CHROME_BIN || '/usr/bin/chromium' });
  const page = await browser.newPage({ viewport: { width: 1440, height: 900 }, userAgent: 'WebsiteTester/0.1 HomeAssistant Playwright' });
  page.on('console', msg => { if (msg.type() === 'error') consoleErrors.push(msg.text()); });
  page.on('requestfailed', req => failedRequests.push({ method: req.method(), url: req.url(), error: req.failure()?.errorText || 'unknown' }));
  const startedMs = Date.now();
  try {
    const response = await page.goto(WEBSITE_URL, { waitUntil: 'domcontentloaded', timeout: TIMEOUT_MINUTES * 60 * 1000 });
    ...
    const images = await page.locator('img:visible').evaluateAll(imgs => imgs.map(img => ({ src: img.currentSrc || img.src, width: img.naturalWidth, height: img.naturalHeight })));
    ...
    if (SCREENSHOT_MODE !== 'none') { result.screenshot = path.join(runDir, 'page.png'); await page.screenshot({ path: result.screenshot, fullPage: true }).catch(() => {}); }
  } finally { result.ended_at = isoNow(); await browser.close().catch(() => {}); }
  const serializable = { ...result, screenshot: result.screenshot ? path.relative(runDir, result.screenshot) : null, console_errors: consoleErrors.slice(0, 50), failed_requests: failedRequests.slice(0, 50) };
...
function delayUntilNextRun() {
  ...
  const target = new Date(localNow); target.setHours(hour, minute, 0, 0);
  if (target <= localNow) target.setDate(target.getDate() + 1);
  return Math.max(1000, target - localNow);
}

async function main() {
  log(`Configured URL: ${WEBSITE_URL}`); log(`Schedule: ${SCHEDULE} (${TIMEZONE})`); log('Send "run" on stdin for an immediate test.');
  let timer;
  const scheduleNext = () => { const delay = delayUntilNextRun(); log(`Next run in ${Math.round(delay / 1000)} seconds`); clearTimeout(timer); timer = setTimeout(async () => { await runTest().catch(error => log(`Unhandled test error: ${error.message}`)); scheduleNext(); }, delay); };
  scheduleNext();
  let buffer = ''; process.stdin.setEncoding('utf8');
  process.stdin.on('data', async chunk => { buffer += chunk; const lines = buffer.split(/\r?\n/); buffer = lines.pop() ?? ''; for (const line of lines) { const command = line.trim().toLowerCase(); if (['run', 'test', 'check'].includes(command)) await runTest().catch(error => log(`Unhandled test error: ${error.message}`)); if (command === 'status') { const latest = path.join(RESULTS, 'latest.json'); console.log(fs.existsSync(latest) ? fs.readFileSync(latest, 'utf8') : JSON.stringify({ status: 'NO_RUN' })); } } });
  await new Promise(() => {});
}
